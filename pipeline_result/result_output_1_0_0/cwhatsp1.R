...
rplot_cwhatsp1 <- function(
  ...
  total_nb <- n_distinct(select(data, lens_id) %>% collect())
  total_nb <- total_nb - nb_missing_category
  total_nb <- format(total_nb, big.mark = ",", scientific = FALSE)
  ...
  return(list(plot = eda_plot, table = data_table, hero=disciplines_nb))
