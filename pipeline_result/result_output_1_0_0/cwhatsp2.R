# ==============================================================================
# Title:        Plot Share of research by document type  
...
rplot_cwhatsp2 <- function(
  ...
  max_nb <- format(max(cleaned_data$nb_records), big.mark = ",", scientific = FALSE)
  ...
  top10_data <- cleaned_data %>%
    arrange(desc(share_records)) %>%
    slice_head(n = 10)  
  
  eda_plot <- ggplot(top10_data, aes(x= reorder(publication_type, share_records), y= share_records, fill=publication_type)) +
  ...
  data_table <- cleaned_data %>%
    select(publication_type, share_records, nb_records, avg_nb_authors, median_nb_authors) %>%
    arrange(desc(nb_records)) %>%
    mutate(
  ...
  return(list(plot = eda_plot, table = data_table, hero=max_nb))
